...
q_s <- function(T_i) {
  size <- length(T_i)
  T_sort <- sort(T_i)
  return (T_sort[ceiling(0.95*size)]+(T_sort[floor(0.95*size)]-T_sort[ceiling(0.95*size)])*(0.95*n-ceiling(0.95*n)))
}

#Estimator of a_m and b_m, using the method of moments
q_m <- function(T_i) {
  #On cherche une racine ? la premi?re ?quation de 1.2.b pour obtenir a_M, puis on calcule b_M et q_M
  f1 <- function(a) {
...
q_l <- function(T_i) {
  #On cherche une racine ? la premi?re ?quation de 1.2.c pour obtenir a_L, puis on calcule b_L et q_L
  f1 <- function(a) {
...
generate_estimation_q_p <- function(affichage) {
  ...
  #Approximation de l'esp?rance par la moyenne empirique
  ...
  if (affichage == TRUE) {
    ...
    title(main = paste("n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_s, digits=4)  , " Variance=", round(variance_s, digits=4), " MSE=", round(mse_s, digits=4)))
    boxplot(estimation_q_s, xlab = "q_s", main = paste("n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0))
    title(main = paste("n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_s, digits=4)  , " Variance=", round(variance_s, digits=4), " MSE=", round(mse_s, digits=4)))
    
    
    hist(estimation_q_m,breaks=10, main = "")
    title(main = paste("n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_m, digits=4)  , " Variance=", round(variance_m, digits=4), " MSE=", round(mse_m, digits=4)))
    boxplot(estimation_q_m, xlab = "q_m", main = "")
    title(main = paste("n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_m, digits=4)  , " Variance=", round(variance_m, digits=4), " MSE=", round(mse_m, digits=4)))
    
    
    hist(estimation_q_l,breaks=10, main = "")
    title(main = paste("n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_l, digits=4)  , " Variance=", round(variance_l, digits=4), " MSE=", round(mse_l, digits=4)))
    boxplot(estimation_q_l, xlab = "q_l" , main = "")   
    title(main = paste("n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_l, digits=4)  , " Variance=", round(variance_l, digits=4), " MSE=", round(mse_l, digits=4)))
    
  }
  
  return (c(c(bias_s,bias_m,bias_l),c(variance_s,variance_m,variance_l),c(mse_s,mse_m,mse_l)))
}
