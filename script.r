...
q_l <- function(T_i) {
  ...
  f1 <- function(a) {
    return (1/a + (1/(sum(log(1-T_i^a)))+(1/n))*sum((T_i^(a)*log(T_i))/(1-T_i^(a)))+(1/n)*sum(log(T_i)))
  }
...
### - 1.3 (c) - ###
  ...
  if (affichage == TRUE) {
    ...
    title(main = paste("Histogram of estimated q_s\n", "n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_s, digits=4)  , " Variance=", round(variance_s, digits=4), " MSE=", round(mse_s, digits=4)))
    boxplot(estimation_q_s, xlab = "q_s", ylab="Value")
    title(main = paste("Boxplot of estimated q_s\n","n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_s, digits=4)  , " Variance=", round(variance_s, digits=4), " MSE=", round(mse_s, digits=4)))
    
    
    hist(estimation_q_m,breaks=10, main = "")
    title(main = paste("Histogram of estimated q_m\n","n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_m, digits=4)  , " Variance=", round(variance_m, digits=4), " MSE=", round(mse_m, digits=4)))
    ...
    title(main = paste("Histogram of estimated q_l\n","n =", n, " N =", nbr_replication, " a_0 =", a_0, " b_0 =", b_0), sub= paste("Biais =", round(bias_l, digits=4)  , " Variance=", round(variance_l, digits=4), " MSE=", round(mse_l, digits=4)))
